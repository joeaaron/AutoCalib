...
PaneWidget::PaneWidget(QWidget *parent, const QString& label, const QString& icon, bool onoffbutton, bool solobutton) : QWidget(parent), ui(new Ui::PaneWidget)
{
	...
	if (!label.isEmpty()) {
		...
		ui->labelPaneName->setStyleSheet(QString::fromUtf8(" QLabel {\n""color: rgba(0, 0, 0, 255)\n""}"));
	}
...
void PaneWidget::setWidget(QWidget *widget, bool enabled /*= true*/)
{
	mainwidget = widget;
	...
	mainwidget->setEnabled(enabled);
}

QWidget *PaneWidget::getWidget() const
{
	return mainwidget;
}
